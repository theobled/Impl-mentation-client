...
Auteur ...... : Mehdi Bouziane / Rauthikan
...
typedef struct {
  Nom nomMatiere; // Nom de la matiere
  float note; // Moyenne de la mati�re
  unsigned int UE; // UE corespondant
}Moyenne;

typedef struct {
  Nom nomMatiere; // Nom de la matiere
  Nom nomEpreuve; // Nom de l'epreuve
  unsigned int NumeroSemestre; // Numero du semestre
  float note; // Note obtenu
}Note;

typedef struct {
  Nom PrenomEtudiant; // Nom de l'etudiant
  Note notes[MAX_MATIERES * MAX_EPREUVES * NB_SEMESTRES]; // Tableau de notes de l'etudiant
  int nbNotes; // Nombre de notes de l'etudiant
  float MoyenneSemestre1[MAX_UE]; // Moyennes du premier Semestre
  float MoyenneSemestre2[MAX_UE]; // Moyennes du deuxieme Semestre
  float MoyennesAnnuelles[MAX_UE]; // Moyennes annuelle
}Etudiant;

typedef struct {
  Nom nom; // Nom de l'epreuve
  float coef[MAX_UE]; // Coefficents de l'epreuve par rapport aux nombre
  // d'unite d'enseignement
}Epreuve;

typedef struct {
  Nom nom; // Nom de la matiere
  unsigned int nbEpreuves; // Nombre d'�preuves de la mati�re
  Epreuve epreuves[MAX_EPREUVES]; // Tableau d'�preuves de la mati�re
}Matiere;

typedef struct {
  unsigned int nbMatieres; // Nombre de mati�res du semestre
  Matiere matieres[MAX_MATIERES]; // Tableau de mati�res du semestre
}Semestre;
...
typedef struct {
  ...
  Etudiant etudiants[MAX_ETUDIANTS]; // Tableau d'�tudiants
}Formation;
...
/* help, la commande permet d'afficher les aides n�cessaires pour toutes
*        les commandes disponibles avec les diff�rents arguments.
*        La commande est appel�e quand l'utilisateur rentre au clavier 'help', '?' ou
...
void help() {
  ...
  printf("**                du nom de la mati�re,\n");
  printf("**                du nom de l'�preuve,\n");
  printf("**                d'un coefficient par UE\n");
  printf("**\n");
  printf("** 'coefficients' suivie du num�ro de semestre\n");
  printf("**\n");
  printf("** 'note'         suivie du num�ro de semestre,\n");
  printf("**                du nom d'un �tudiant,\n");
  printf("**                du nom d'une matiere,\n");
  printf("**                du nom d'une �preuve,\n");
  printf("**                d'une note\n");
  printf("**\n");
  printf("** 'notes'        suivie du num�ro de semestre,\n");
  printf("**                du nom d'un �tudiant\n");
  printf("**\n");
  printf("** 'releve'       suivie du num�ro de semestre,\n");
  printf("**                du nom d'un �tudiant\n");
  printf("**\n");
  printf("***************************************************************\n");
}
 ...
 *  La commande est appel�e quand l'utilisateur rentre au clavier 'exit'
 */
int exitcommand() {
  exit(EXIT_SUCCESS);
  return EXIT_SUCCESS;
...
/* verifUE, permet de v�rifier si le nombre d'UE a �t� d�fini
 * [in]  inF, formation
...
/* verifEtudiant, permet de verifier si le prenom de l'�tudiant existe deja
 * [in]  inPrenomEtu, chaine de caract�res contentant le pr�nom de l'�tudiant
 * [in]  inOutF, Formation
 * [in]  inAffichage, Afficher le message 'etudiant inconnu' si inAffichage = 1
 * [return]  Return l'index du tableau etudiants, sinon -1 (etudiant non trouv�)
 */
...
/* verifNoteEtu, permet de verifier si toutes les notes de l'�tudiant ont �t�es ajout�es
 * [in]  inIndexEtu, index de l'�tudiant
 * [in]  inOutF, Formation
 ...
 * [in]  inNumSem, num�ro du semestre
...
/* affichageReleve, permet de d'afficher le releve de note du semestre donn�
                    pour un etudiant
 * [in]  inF, Formation
 * [in]  inIndexEtu, index de l'�tudiant
 * [in]  moyennes, tableau de Moyenne de toutes les mati�res
...
/* CalculReleve, permet de calculer les moyennes de chaque mati�res
 * [in]  inOutF, Formation
 * [in]  inIndexEtu, index de l'�tudiant
 * [in]  inNumsem, num�ro du semestre
 * [in]  inAffichage, Afficher les moyennes de chaque mati�re: inAffichage = 1, sinon inAffichage = 0
 */
...
/* Initialisationformation, permet d'initialiser toutes les structures � partir de formation
 * [out]  inOutF,Formation � initialiser
 */
void Initialisationformation(Formation* inOutF) {
  inOutF->nbUE = 0;
...
/* ajoutMatiereEpreuve, permet de cr�er des mati�res, des �preuves et affecter les coefficients des UEs
 * [in]  inmatierename, chaine de caractere contentant le nom de la mati�re
 * [in]  inepreuvename, chaine de caractere contentant le nom de l'�preuve
...
/* formation, permet de cr�er une formation d'UE
...
/* epreuve, commande epreuve permet de cr�er des mati�res, des epreuves et affecter des coefficients
 * [in-out]  inOutF, Formation
 */
void epreuve(Formation* inOutF) {
...
/* note, commande coefficients permet de verifier la note d'un �tudiant
 * [in-out]  inOutF, Formation
 */
void note(Formation* inOutF) {
  unsigned int UE = verifUE(inOutF);
...
/* notes, commande coefficients permet de verifier les notes d'un �tudiant
 * [in-out]  inOutF, Formation
 */
void notes(Formation* inOutF) {
  unsigned int UE = verifUE(inOutF);
...
/* releve, commande affichant le releve de notes d'un �tudiant
 * [in]  inF, Formation
 */
